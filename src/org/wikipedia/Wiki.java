...
public class Wiki implements Serializable {
  ...
  public synchronized void undelete(String title, String reason, Revision... revisions)
    ...
    String delrev = query + "action=query&list=deletedrevs&drlimit=1&drprop=token&titles="
        + titleenc;
  ...
  public String[] getLinksOnPage(String title) throws IOException {
    ...
    do {
      ...
      if (plcontinue == null)
        ...
        line = fetch(url.toString() + "&plcontinue=" + URLEncoder.encode(plcontinue, "UTF-8"),
            "getLinksOnPage");
  ...
  public String[] getExternalLinksOnPage(String title) throws IOException {
    ...
    do {
      ...
      if (eloffset == null)
        ...
        line = fetch(url.toString() + "&eloffset=" + URLEncoder.encode(eloffset, "UTF-8"),
   ...
   * @param reverse
   *          whether to put the oldest first (default = false, newest first is how history pages
   *          work)
  ...
  public Revision[] getDeletedHistory(String title, Calendar start, Calendar end, boolean reverse)
    ...
    do {
      ...
      if (drvcontinue != null)
        response = fetch(
            url.toString() + "&drvcontinue=" + URLEncoder.encode(drvcontinue, "UTF-8"),
            "getDeletedHistory");
      else
      ...
      for (x = response.indexOf("<page ", x); x > 0; x = response.indexOf("<page ", ++x)) {
        ...
        for (int z = response.indexOf("<rev ", x); z < y && z >= 0; z = response.indexOf("<rev ",
            ++z)) {
  ...
  public Revision[] deletedContribs(String u, Calendar end, Calendar start, boolean reverse,
    ...
    do {
      ...
      if (adrcontinue != null)
        response = fetch(
            url.toString() + "&adrcontinue=" + URLEncoder.encode(adrcontinue, "UTF-8"),
            "deletedContribs");
      else
   ...
   *
   * @param prefix
   *          a prefix without a namespace specifier, empty string lists all deleted pages in the
   *          namespace.
   * @param namespace
  ...
  public String[] deletedPrefixIndex(String prefix, int namespace) throws IOException,
    ...
    do {
      ...
      if (drcontinue == null)
        ...
        text = fetch(url.toString() + "&drcontinue=" + URLEncoder.encode(drcontinue, "UTF-8"),
            "deletedPrefixIndex");
   ...
   * @throws UnsupportedOperationException
   *           if the original page is in the Category namespace. MediaWiki does not support moving
   *           of these pages.
   * @throws IOException
   *           if a network error occurs
   * @throws CredentialNotFoundException
   ...
   *
   * @param hidecontent
   *          hide the content of the revision (true/false = hide/unhide, null = status quo)
   * @param hidereason
   ...
   * @param suppress
   *          [[Wikipedia:Oversight]] the information in question (ignored if we cannot
   ...
   * @param ns
   *          a list of namespaces to filter by, empty = all namespaces.
   * @return the contributions of the user, or a zero length array if the user does not exist
   * @throws IOException
   *           if a network error occurs
   * @since 0.17
   */
   ...
   * @return contributions of this user, or a zero length array if the user does not exist
   * @throws IOException
   *           if a network error occurs
   * @since 0.17
   */
    ...
    do {
      ...
      if (line.contains("uccontinue"))
        uccontinue = "&uccontinue="
            + URLEncoder.encode(parseAttribute(line, "uccontinue", 0), "UTF-8");
      else
  ...
  public String[] getRawWatchlist(boolean cache) throws IOException, CredentialNotFoundException {
    ...
    do {
      ...
      if (wrcontinue == null)
        ...
        line = fetch(url + "&wrcontinue=" + URLEncoder.encode(wrcontinue, "UTF-8"),
            "getRawWatchlist");
   ...
   * @param subcat
   *          do you want to return members of sub-categories also? (default: false) Recursion is
   *          limited to a depth of one.
   * @param ns
   *          a list of namespaces to filter by, empty = all namespaces.
   * @return a String[] containing page titles of members of the category
   * @throws IOException
   ...
   * @param namespace
   *          filters by namespace. Returns empty if namespace doesn't exist. Use ALL_NAMESPACES to
   *          not specify one.
   * @throws IOException
   *           if a network error occurs
   * @throws IllegalArgumentException
...
/**
  ...
  public class Revision implements Comparable<Revision> {
    ...
    /**
     * Returns true if the revision content is RevisionDeleted. WARNING: the return value is
     * meaningless until <tt>getRenderedText</tt>, <tt>getText</tt> or <tt>diff()</tt> is called
     * because of MW api limitations.
     *
     * @return (see above)
     * @since 0.31
     */
    ...
    /**
     * Returns the edit summary for this revision, or null if the summary was RevisionDeleted and
     ...
     * <tt>getUser(String)</tt> to obtain a User object. Returns null if the user was
  ...
  protected String fetch(String url, String caller) throws IOException {
    ...
    // get the text
    String temp;
    try (BufferedReader in = new BufferedReader(new InputStreamReader(zipped ? new GZIPInputStream(
   ...
   * @param api
   *          whether the timestamp is of the format yyyy-mm-ddThh:mm:ssZ as opposed to
  ...
  public void fastEdit(final String article, final String text, final String comment)
